...
### Шаг 5 — Post-dispatch валидация

После завершения Task() пайплайна — проверь отчёт:

1. Прочитай финальный отчёт пайплайна (последнее сообщение subagent)
2. Проверь наличие признаков пропуска фаз:
   - Есть ли в отчёте слова: "пропущен", "пропущена", "SKIP" (кроме обоснованных SKIP)
   - Есть ли фаза REVIEW в отчёте (для new-code, fix-code, tests, hotfix)
   - Есть ли фаза CAPTURE в отчёте

Если обнаружены проблемы:
AskUserQuestion:
  question: "⚠️ Обнаружены проблемы в выполнении пайплайна:\n{список проблем}\nПовторить пайплайн с соблюдением всех фаз?"
  options:
    - {label: "Повторить", description: "Запустить пайплайн заново с полным выполнением всех фаз"}
    - {label: "Принять как есть", description: "Принять текущий результат"}
