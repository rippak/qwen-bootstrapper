...
## Правила работы с инструментами (ОБЯЗАТЕЛЬНО для ВСЕХ агентов)

Прочитай `templates/includes/tool-usage-rules.md` и вставь содержимое в КАЖДЫЙ сгенерированный агент
как секцию `## Правила работы с инструментами` после `## Контекст` и перед `## Задача` / `## Правила`.

## Анти-лень (ОБЯЗАТЕЛЬНО для ВСЕХ агентов)

Прочитай `templates/includes/anti-laziness.md` и вставь содержимое в КАЖДЫЙ сгенерированный агент
как **ПЕРВУЮ секцию** сразу после `## Режим` (или после `## Контекст` если `## Режим` отсутствует).
Эта секция должна идти ПЕРЕД всеми остальными инструкциями.

Это ОБЯЗАТЕЛЬНО для:
- ВСЕХ языко-специфичных агентов (architect, developer, test-developer, reviewer-logic, reviewer-security)
- ВСЕХ общих агентов (analyst, db-architect, devops, frontend-*, qa-engineer)
- ВСЕХ кастомных агентов (CUSTOM_AGENTS)

## Подстановка переменных (CRITICAL)

При генерации ВСЕХ файлов (агенты, скиллы, пайплайны) **ЗАМЕНИ все placeholder'ы** на реальные значения:

### Переменные из Step 1 (Анализ)

| Placeholder | Источник | Описание |
|-------------|----------|----------|
| `{lang}` | LANGS | Язык: php, node, python, etc. |
| `{LANG}` | LANGS | Язык в верхнем регистре: PHP, NODE, PYTHON |
| `{SOURCE_DIR}` | Анализ | Путь к исходникам: courts/app/, src/, app/ |
| `{TEST_DIR}` | Анализ | Путь к тестам: courts/tests/, tests/ |
| `{MIGRATIONS_DIR}` | Анализ | Путь к миграциям: courts/database/migrations/ |
| `{FRAMEWORK_{lang}}` | Анализ | Фреймворк: laravel, express, django |
| `{TEST_FRAMEWORK_{lang}}` | Анализ | Тест-фреймворк: phpunit, jest, pytest |
| `{TEST_CMD_{lang}}` | Анализ | Команда тестов |
| `{LINT_CMD_{lang}}` | Анализ | Команда линтера |
| `{CONTAINER_CMD}` | Анализ | Docker/podman команда |

### Переменные пайплайнов (генерируются в step-4b, но определи сейчас для агентов)

| Placeholder | Значение |
|-------------|----------|
| `{MIGRATE_CMD}` | Команда миграций (определи по CONTAINER + FRAMEWORK) |
| `{TEST_CMD}` | Команда тестов (из `{TEST_CMD_{lang}}`) |
| `{SYNTAX_CHECK_CMD}` | Команда синтаксиса (из `{LINT_CMD_{lang}}` или аналог) |

**ПРАВИЛО:** Если placeholder нечем заменить — определи лучшее значение по контексту, НЕ ОСТАВЛЯЙ как есть.
